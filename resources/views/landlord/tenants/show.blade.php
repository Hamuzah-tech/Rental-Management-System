...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div class="bg-white border border-[#E5E7EB] rounded-xl p-4">
            ...
            <p class="text-sm font-mono text-[#111827] mt-1">{{ e($tenant->tenant_code) }}</p>
        </div>
        <div class="bg-white border border-[#E5E7EB] rounded-xl p-4">
            <p class="text-xs text-[#6B7280]">Property</p>
            <p class="text-sm font-medium text-[#111827] mt-1">{{ e($tenant->property->name ?? 'N/A') }}</p>
        </div>
        <div class="bg-white border border-[#E5E7EB] rounded-xl p-4">
            <p class="text-xs text-[#6B7280]">Monthly Rent</p>
            <p class="text-sm font-bold text-[#111827] mt-1">MK {{ number_format((float)($tenant->monthly_rent ?? 0)) }}</p>
        </div>
        <div class="bg-white border border-[#E5E7EB] rounded-xl p-4">
            <p class="text-xs text-[#6B7280]">Status</p>
            <span class="inline-block mt-1 px-2.5 py-0.5 rounded-full text-xs font-medium capitalize
    ...
    <div class="bg-white border border-[#E5E7EB] rounded-xl overflow-hidden">
        ...
        <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <p class="text-sm text-[#111827] font-medium">{{ e($tenant->name) }}</p>
            </div>
            <div>
                <p class="text-xs text-[#6B7280]">Email</p>
                <p class="text-sm text-[#111827]">{{ e($tenant->email) }}</p>
            </div>
            <div>
                <p class="text-xs text-[#6B7280]">Phone Number</p>
                <p class="text-sm text-[#111827]">{{ e($tenant->phone) }}</p>
            </div>
            <div>
                        ...
                        <tr class="border-t border-[#E5E7EB] hover:bg-[#F8FAFC] transition">
                            ...
                            <td class="px-4 py-3">
                                @if($payment->status == 'Pending')
                                    <span class="inline-block bg-[#ca0251] text-white px-2.5 py-0.5 rounded-full text-xs font-medium">
                                        Pending
                                    </span>
                                @elseif($payment->status == 'Approved')
                                    <span class="inline-block bg-green-600 text-white px-2.5 py-0.5 rounded-full text-xs font-medium">
                                        Approved
                                    </span>
                                @elseif($payment->status == 'Rejected')
                                    <span class="inline-block bg-red-600 text-white px-2.5 py-0.5 rounded-full text-xs font-medium">
                                        Rejected
                                    </span>
                                @else
